...
howells$Sex <- dplyr::recode_factor(howells$Sex, 'M' = 'Male', 'F' = 'Female')
...
names(howells) <- tolower(names(howells))
```
...
names(howells) %<>% toupper(names(howells))
```

* Columns

**PSA: Will change back to a STRING - AKA may need to change back to a factor**

```{r}
#| echo: true
#| eval: false

howells$Population %<>% tolower() %>% factor()
```
...
1.  How many individuals are in each population and sex group? Output the table into a .csv. HINT: write_csv(object, “FileName.csv”)

2.  Create a table that includes population, sex, the mean of GOL, the sd of GOL, and the sample size. 
...
samplesize <- howells %>% group_by(Population, Sex) %>% summarize(count = n())
...
write_csv(samplesize, "howells_sampSize.csv")
```
...
howSumStats <- howells %>% group_by(Population, Sex) %>% summarize(mean(GOL), sd(GOL), `Sample Size` = n())
