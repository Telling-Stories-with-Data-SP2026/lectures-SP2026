---
title: "Introduction to Data"
subtitle: "Spring 2026"
author: ""
format: 
  revealjs:
    theme: moon
    slide-number: true
    show-slide-number: all
    chalkboard: false # Great for drawing during stats lectures
    incremental: false # Bullet points appear one by one
    embed-resources: true
    df-print: kable
editor_options: 
  chunk_output_type: console
---

## To start class

1. Clone your week 2 repo to your machine.
2. Open a file to keep notes in - either a qmd or script.
3. Ensure file is saved in the project folder per week.
4. Repeat.

## What the **** are data?

***Data are a means to represent the world***

![](data_types.png)

# Context Matters

## Semantics

* Row = observation, record, case, example, instance, pattern, sample
* Columns = variable, field, feature, attribute, input, predictor, dimension

![](tabular.jpg){fig-align="center"}

## 

![](data_info.png){fig-align="center"}

## Categorical Data

* **Nominal:** Unordered category
* **Ordinal:** Ordered category
* Both can be [binary]{.underline} or [multinomial]{.underline}

![](cat_data.jpg){fig-align="center"}

## Numeric Data

* **Continuous:** Can take on any number
  * **Interval:** Distance between values are equal and meaningful
    * Numbers are 'arbitrary' and lack a 0 point
    * IQ, temperature, etc.
  * **Ratio:** Defined 0 point. Cannot fall below 0.
* **Discrete:** Can only take on certain numbers. There are 'gaps' between numbers. 
  * Counts & Integers (whole numbers)
  
##

![](quant.png){fig-align="center"}

## A Note about Research Design

* **Qualitative Research:** Descriptive statements to seek answers
* **Quantitative research:** measurements to seek answers from qualitative or quantitative data
  * ***Data Science***
  


* **Less precise:** Qualitative / categorical
* **More precise:** Quantitative / continuous

# Data Types & R

## Common Data Types

| Type     | Definition | Example |
|----------|----------|----------|
| Double   | Whole or floating number | 5 or 5.73|
| Integer  | Whole number             | 5, 2, 3L |
| Character| Individual or strings of non-numbers|"c", "cat", "cat in the hat" |
| Factor   | Categorical or discrete variables | M/F, S/M/L |
| Boolean  | Binary Categories | T/F |

## Data Type in R

> Numbers

```{r}
# Numbers
c(4.12, 4.57, 5.00, 17.00)
```

> Characters

```{r}
c("M", "male", "F", "cat", "Cat-Dog")
```

> Factors

```{r}
factors <- as.factor(c("M", "M", "F", "M"))
factors
```

> Boolean

```{r}
c(TRUE, FALSE, T, F)
```

## Special Data Types

\
\
\

```{r}
NULL

NA

NaN

Inf
```

## Data Modes

Each variable / object has a data mode that umbrellas by data type.

::::{.columns}

::: {.column width = "50%"}
**Numeric:**
* Both *integers* and *doubles*
* Includes *factors*

**Character:**
* *Characters* and *strings*

**Logical:**
* Boolean TRUE and FALSE

:::

:::{.column width = "50%"}

The `mode()` function returns the type of data mode. 

```{r}
#| echo: true

mode(42)

a <- "beer"
mode(a)

mode(T)

mode(as.factor("M"))
```


:::

::::

## Checking and Converting

![](modes.png){fig-align="center"}

##

::::{.columns}

:::{.column width = "50%"}

\
\
\

```{r}
#| echo: true

is.numeric(2)
```

```{r}
#| error: true
#| echo: true

is.numeric(a)
```

```{r}
#| echo: true

is.character("a")
```

```{r}
#| echo: true

as.character(4)
as.numeric(4)
```

:::

:::{.column width = "50%" font-size = "smaller"}


* It is often useful to make discrete categories (strings) into a ***factor***. This allows for ease in analyses and visualizations. 
  
```{r}
#| echo: true

fac <- c("M", "F")
mode(fac)
fac <- factor(fac)
fac
is.factor(fac)
```

:::

::::

# Data Structures

## But First! Importing Data!

* **Option 1:** Environment Pane > Import
  * Point and click

* **Option 2:** Code!

```{r}
#| echo: true
#| eval: false

data <- read.csv(...) # Base R

library(readr)
read_csv(...)

library(readxl)
read_xls(...)
```

```{r}
#| echo: true

dat <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/main/data/2025/2025-04-01/pokemon_df.csv')
```

##

```{r}
dplyr::glimpse(dat)
```

## Data Structures

![](data_structures.jpg){fig-align="center"}

## Vector == 1 Dimension

* All elements have the same `mode`.

::::{.columns}

::: {.column width = "50%"}

```{r}
dat[,c("pokemon", "type_1")] |> head()
```


:::

::: {.column width = "50%"}

* The `c()` function combines arguments into a vector.

```{r}
#| echo: true

v <- c(1, 2, 3)
w <- c(10, 11, 12)
c(v, w)
```


```{r}
#| echo: true

vec <- c("this", "is", "a", "vector")
vec
```

:::

::::

## Matrix == 2 dimensions, same mode

Vectors combined together.

::::{.columns}

::: {.column width = "60%"}

```{r}
dat[,c("pokemon", "type_1", "type_2")] |> head()
```


:::

::: {.column width = "40%"}

* `matrix()`  combines vectors.

`matrix(data=NA, nrow=1, ncol=1, byrow=F, dimnames=F)`

```{r}
#| echo: true

matrix(c(v, 4, 5, 6), nrow=2, ncol=2, byrow=T)

```

:::

::::

## Data Frames == 2 dimensions, different modes

* ***AKA a tibble***
* Most common data R data structure.
* Rectangular data [Rows X columns]
* Each column has a ***single*** mode.(AKA column = vector)

```{r}
dat[,c("pokemon", "type_1", "hp")] |> head( n = 3)
```

##

![](df.png){fig-align="center"}

## Arrays and Lists: 3+ Dimensions

* **Array:** = Many matrices into 1 object / container.
  * 1 data mode
  
* **List:** = Combination of all data structures
  * Multiple data modes & types
  * ***Complex*** but incredibly [***useful***]{.underline}
  
* Remember, there is a `function` for that!

```{r}
#| eval: false
#| echo: true

array(...)
data.frame(...)
list(...)

# is.[...] / as.[...]
```

# Note!

* ***R*** is a computer programming [language]{.underline}

  * Languages have [**dialects**]{.underline}
  
    * ***Primary / Base:*** Pre-loaded
    * ***Others:*** Pull from `library()`
    
* You have to walk before you can run... we will start with `base`...
* Remember ***R*** can only do what you ask it! It is [***hyper***]{.underline} literal

# Exploring Data

## Take a Peak {.smaller}

* `head(x, n=6): return the first n elements`
* `tail(x, n=6): return the last n elements`

```{r}
#| echo: true

head(dat, n = 3)
```

```{r}
#| echo: true

tail(dat, n = 2)
```

## Check the Structure

* `dim(x)`: return the dimensions of an object 
  * Matrix, data frame, or array
  * rows, columns, (depth)

```{r}
#| echo: true

dim(dat)
```

```{r}
#| echo: true

mat <- matrix(c(1, 2, 3, 4, 5, 6), nrow=3, ncol=2)
dim(mat)
```

```{r}
#| echo: true
#| error: true

## WARNING

a <- c(1, 2, 3, 4)
dim(a)
```

## Check the Structure

* Universal check

```{r}
#| echo: true

str(a)
```


```{r}
#| echo: true

str(dat)
```

## Indexing Data

:::: {.columns}

::: {.column width = "40%"}

* **Index:** = an address to a value(s)

\

```{r}
#| echo: true

# Vectors
b <- c(1, 3, 5, 7, 9)
b
b[3]
```

:::

::: {.column width = "60%"}

* **Matrices / DF:**
  * 2 dimensions == 2 index values
  * ***ALWAYS ROWS x COLUMNS***
  
![](index.png)

:::

::::

##

```{r}
head(dat, 3)
```

```{r}
#| echo: true

dat[2,1]
```

```{r}
#| echo: true


dat[2, 1:3]
```

## Get a Summary

* `summary()` returns the summary statistics for an object.

```{r}
#| echo: true
summary(dat[,c(2,4,5)])
```

# Reformatting Data

## Names

* `names()`: Dataframe or list
* `colnames()`: Dataframe or matrix

```{r}
#| echo: true

colnames(dat)
```


***NB:*** These can be useful to get index values of columns.

## Selecting Columns - 1 Column {.smaller}

***Remember, a column of data is a vector with 1 mode.***

* Some functions may require you to input a vector
* To select only 1 column:
  * Matrix:
    * Index notation - e.g., `dat[,5]`
  * Dataframe:
    * `$` - e.g., `dat$hp`
    * Index notation 1 - e.g., `dat[[9]]` or `dat[["hp"]]`
      * Note - the goal here is to return a **vector**. You can use index notation such that `dat[,5]` but it will return a dataframe and not a vector. 
      
##

```{r}
#| echo: true

head(dat$pokemon)
```

```{r}
#| echo: true

head(dat[["pokemon"]])
```

```{r}
#| echo: true

head(dat[[2]])
```

```{r}
#| echo: true

head(dat[2], n = 3)
```

## Selecting Columns 2+

***Maintains dataframe-ness or matrix shape***

* Always using **Index Notation** `[ ]`

```{r}
#| echo: true

head(dat[c(1,4,5)], n = 2)
```

```{r}
#| echo: true

head(dat[,c(1,4,5)], n = 2)
```

##

```{r}
#| echo: true

head(dat[,3:7])
```

##

```{r}
#| echo: true

summary(dat[,4:7])
```

```{r}
#| echo: true

dat$type_1 <- as.factor(dat$type_1)

summary(dat[,4:7])
```


## Renaming Columns

* Remember `names()` or `colnames()`. 

```{r}
#| echo: true

names(dat)
```

```{r}
#| echo: true


names(dat)[5] <- "WT"

names(dat)
```

##

```{r}
#| echo: true

names(dat)[4:6] <- c("HT", "WGT", "EXP")
names(dat)
```

```{r}
#| echo: true


names(dat)[c(9, 11, 14)] <- c("HP", "DEF", "SPD")
names(dat)
```

## Recoding a Single Value {.smaller}

* Remember how to find the 'address' of a certain value in your data. 
  
```{r}
#| echo: true

dat[1:3, c(2, 4, 5)]
```

```{r}
#| echo: true

dat[3,4] <- NA

dat[1:3, c(2, 4, 5)]
```

## Combining Rows {.smaller}

* `rbind()` combines data **vertically**
  * All datasets to be combined must share the ***same number*** of columns
    *  Note, `rbind` will work if columns are out of order but the number is the same. It is up to the User to ensure they match.
    
```{r}
#| echo: true

dat1 <- dat[1:2, 1:3]
dat2 <- dat[7:9, 1:3]

dim(dat1)
dim(dat2)

```

```{r}
#| echo: true
dat3 <- rbind(dat1, dat2)
dim(dat3)
```

## Combining Columns {.smaller}

* `cbind()` combines data **horizontally**
  * Think of this as putting together 2 datasets with different columns but **the same rows**
    * I.E. - You are adding on features that were collected per observation

```{r}
#| echo: true

dat4 <- dat[1:2, c(2, 7)]
dat5 <- dat[1:2, 4:6]

colnames(dat4)
colnames(dat5)

```

```{r}
#| echo: true
dat6 <- cbind(dat4, dat5)
colnames(dat6)
```

## Combining Columns Using Paste()

* Let's say you have features in your dataset you want to aggregate into a new identifier. 
  * `Sex` + `Population` 
  * `Age_Group` + `Political_Party`
  
```{r}
#| echo: true

# Let's make a new object for ease

poke2 <- dat[,c(2,7)]
str(poke2)
```

##

```{r}
#| echo: true

# Now let's make a new column!

poke2$name_type <- paste(poke2$pokemon, poke2$type1, sep = "_")
colnames(poke2)
```

```{r}
#| echo: true

head(poke2, n = 3)
```

##

![](baser.png){fig-align="center"}