...
> Estimating the relationship between some outcome variable ($y$) and some independent variable ($x$).

![](reg_def.png){fig-align="center"}
...
* Residuals
  ...
  * Difference observed $y$ and predicted $\hat{y}$
...
1.  The relationship between a [scalar]{.underline} response variable and a [scaler]{.underline} independent variable is linear.
2.  The relationship between a [scalar]{.underline} response variable and some discrete independent variable where the coefficient represent the difference between group(s)
3.  Generally, the relationship between some response variable and a [***linear combination***]{.underline} of predictors commonly referred to as $y = \beta_0 + \beta_1X_1 + \beta_2X_2 ... + \beta_KX_K$
...
* 1 scalar response $y$ and scalar predictor $x$
* Must adhere to all parametric assumptions
* **Intercept:** The predicted value of $y$ when $x = 0$
* **Coefficients:** The unit change in $y$ when $x$ changes by 1 unit.
* **$R^2$:** Proportion of variability in $y$ explained by $x$
* **P-Values:** Always assume the [null]{.underline} is no effect.
...
summary(lm(bill_len ~ flipper_len, data = penguins))
```
...
*  2+ predictors ($\mathbf{X}$)
  * 2+ coefficients or slopes
  
* Coefficient interpretations
  * Hold 'extraneous' values constant (set equal to average or 0)
  
* Intercept interpretations
  * **If continuous:** Value of $y$ when $\mathbf{X}$ is held constant or at 0.
  * **If discrete:** Value of $y$ at reference value(s).
...
summary(lm(bill_len ~ flipper_len + body_mass + species + sex, data = penguins))
```
...
* Traditionally the ***logit*** link function 

* Results are on log odds scale

* To interpret on *raw* scale, must exponentiate the coefficient $e^{\beta_{k}}$
  * $>$ 1 = increase in odds
  * < 1 = decrease in odds
...
fit <- glm(as.factor(type) ~ bp + bmi + glu, data = MASS::Pima.tr, family = binomial(link = "logit"))
summary(fit)
exp(fit$coefficients)
...
## Probit Regression{.smaller}

:::{.panel-tabset}

## Interpretation

* **probit** link function instead of **logit**
  * Underlying data is related to the normal distribution instead of the logistic distribution

* Use when you have [theoretical]{.underline} reasoning to assume a normal distribution

* Easy to interpret, more complicated to fit

* Results often similar to *logit*

* Model scale is a z-score. 
  * Must take the inverse `pnorm()` to interpret on probablity scale.
  * Using `pnorm` alone is not intuitive outside of the intercept (baseline probability). If we want to understand the probablity of some single effect we must estimate [marginal]{.underline} effects. 
...
fit2 <- glm(as.factor(type) ~ bp + bmi + glu, data = MASS::Pima.tr, family = binomial(link = "probit"))
summary(fit2)
...
* Count data
* Mean = variance
  * Assumes variance is homogenous
* Poisson distribution
* Log link
* Model scale = log count or rate
  * Must exponentiate to get incidence rate ratio (IRR) or rate increase/decrease in count
...
library(AER)
data("Affairs")
fit3 <- glm(affairs ~ gender + yearsmarried + children, data = Affairs, family = poisson)
summary(fit3)
```
...
* Count data
* Generalization of Poisson to over/under dispersed data
* Interpretation is same as Poisson
...
fit4 <- glm.nb(affairs ~ gender + yearsmarried + children, data = Affairs)
summary(fit4)
```
...
* Count data
  ...
  * number successes (or failures) against total # of trials
  * Binary logistic regression is a special case where trials = 1
  
* Estimating probability of success / failure

* The $y$ variable is a matrix of successes/failures and total - successes/failures 
...
## successes = ndead, failures = ntotal - ndead
model <- glm(cbind(ndead, ntotal - ndead) ~ dose + sex, 
             data = doBy::budworm, 
             family = binomial)
summary(model)
```
...
* $y$ variable is many categories

* Interpretation is same as binary logistic regression

* Can be logit or probit link function

* Multi-category classification

* Interpret in relation to reference category
...
library(nnet)
multi <- multinom(Species ~ Sepal.Length + Sepal.Width + Petal.Length + Petal.Width, data = iris)
```
...
* Logit or probit
  * (Log) odds or (z-score) probability

* **Coefficient interpretation:** A one unit change in a predictor represents the change in odds of being in a higher category vs lower category

* **Intercept interpretation:** Threshold parameter. The odds or probablity of being in one category vs the next. 

  * When using ordered regression in a scenario where your predictor is continuous (i.e., age), can be representative of threshold where we move from one stage to next. 
  
* Useful in classification / regression problems
...
ord <- polr(rating ~ temp + contact, data = dat)
...
# The odds of unit change in a predictor leading to an increase in category
exp(ord$coefficients)

# The odds of being in X category
exp(ord$zeta)
```
...
* Data are structured into groups or clusters [nested]
  * **Varying slopes:** different relationships between variables aross groups
  * **Varying intercepts:** Different group baselines
  
![](mlm.png){fig-align="center"}

## Non-Linear Models{.smaller}

* Remember, the goal of regression is simply predicting *y*
  * Finding the best fit line
  * Depending on your problem, your [parameters]{.underline} may be interpretable.
* The regression equation is simply a ***mean!*** [or similar parameter of interest]
...
# Example data: Height of a plant over time
days <- c(1, 5, 10, 20, 30, 40, 50, 60)
height <- c(2, 8, 15, 35, 55, 65, 72, 75)
plot(days, height)
...
# a and b are the parameters we want R to find
fit <- nls(height ~ (a * days) / (b + days), 
           start = list(a = 80, b = 25))
...
lines(days, predict(fit), col="red", lwd=2)
...
* **Locally Weighted Regression [LOESS]**: Flexible line fitting

![](loess.gif){fig-align="center"}

## Non-Parametric Models

* **Splines:** sum of many localized pieces
  * Areas of change are called *knots*

![](splines.png){fig-align="center"}

## Gaussian Process

* A probability distribution over possible functions.

* Instead of finding one "best-fit" line, it predicts a range of likely functions that pass near observed data points, providing both predictions and uncertainty estimates. 

![](gp.gif){fig-align="center"}
...
![](linear_nn.png){width=80%}

## Logistic Regression

![](logistic_nn.png){width=40%}
...
* Always fit more than 1 regression model

* Model comparison is an art

  * In AIC, lower is best.
  * $R^2$ talks about variation but is not useful in comparison

* Useful in prediction / classification 

* Prediction intervals are **always** larger than confidence intervals

* All regression has a tendency to ***regress to mean***
  * It is important we collect appropriate data. 
